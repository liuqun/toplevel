...
const char * const g_RootCACert = "./root-ca.pem";
const char * const g_ServerCert = "./server-cert.pem";
const char * const g_ServerKey = "./server-key.pem";
...
int main()
{
	...
	rv = SSL_CTX_load_verify_locations(ctx, g_RootCACert, NULL);
	CHK_RV(rv, "SSL_CTX_load_verify_locations");

	rv = SSL_CTX_use_certificate_chain_file(ctx, g_ServerCert);
	CHK_RV(rv, "SSL_CTX_use_certificate_chain_file");

	rv = SSL_CTX_use_PrivateKey_file(ctx, g_ServerKey, SSL_FILETYPE_PEM);
	...
	socketAddrServer.sin_port = htons(4433);
